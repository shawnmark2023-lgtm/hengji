...
- AES-256-GCM 受保护账本仓储、Windows/Android/iOS 原子密文文件适配器，以及可中断恢复的 Desktop Room 明文迁移门禁。
...
首版无登录、默认不联网，不采集姓名、手机号、证件、通讯录、位置、广告标识或设备指纹。交易和资产记录即使不含直接身份字段，仍按敏感财务数据处理。工程已实现平台密钥保护、认证密文仓储与 Desktop 明文迁移门禁，但当前应用入口仍明确使用 Room 明文开发存储；切换生产默认值前还必须完成 Android/iOS 旧库迁移、真机运行和代表性数据量性能验收。
...
- 平台密钥真机验收、Android/iOS 旧 Room 库迁移、应用入口切换、加密同步、灾难恢复和真实平台授权尚未完成；密文仓储并不把现有 bundled SQLite 文件原地变成密文。
