...
| ID | 状态 | 关键依赖 | 可量化验收 |
| --- | --- | --- | --- |
| FND-003 | BLOCKED | Git remote、独立 CI runner | 本地 Windows/Android 各在两个隔离 ASCII 源码副本构建并比较规范化 archive 内容；PR CI 已包含 Desktop、Android host/debug、API 36 instrumentation 和 Windows MSI 作业。仓库无 remote，无法产生独立 runner 通过证据 |
