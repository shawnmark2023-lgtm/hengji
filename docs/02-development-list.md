...
- [ ] `DAT-004` P1 已实现跨平台 AES-256-GCM 账本封装及 Windows DPAPI、Android Keystore、iOS/macOS Keychain 数据密钥保护；Desktop 入口与可恢复明文迁移已接入，Android/iOS 入口、移动端旧库迁移及平台运行验收仍是 Beta 门禁。
...
| DAT-004 | PARTIAL | Android/Apple 平台 runner、移动端旧库迁移 | AES-256-GCM envelope 已绑定版本/算法/密钥别名；Windows DPAPI、Android Keystore 与 iOS/macOS Keychain 边界均已实现且不允许明文降级；Desktop 入口、Room 明文→密文复制迁移及中断恢复已接入；仍需 Android/iOS 入口和迁移、Android/Apple 运行验收、轮换与灾难恢复 |
