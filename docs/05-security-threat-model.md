...
| T04 | 重复导入、并发提交、部分写入 | 重复支出/审计困难 | 版本化稳定指纹；批内与账本去重；软删除墓碑仍占用指纹；`ImportLedger` 规定提交/撤销原子性 | SQLite 事务与故障注入测试 |
| T05 | 导入撤销误删用户后来修改的数据，或删除/撤销使退款失去原交易 | 数据丢失、账本引用失真 | 批次保存不可变记录 ID；只撤销该批插入项；存在批外活跃退款时原子拒绝回滚；活跃退款阻止删除原交易，退款恢复要求原交易活跃 | 冲突文案、批内/批外退款矩阵、平台持久层故障注入 |
...
| T11 | 恶意/被入侵报价提供方返回错币种、负数、旧数据 | 估值操纵 | 整数、币种、时区、置信度、未来时间校验；所有仓储拒绝资产/报价跨币种；客户端硬排除超过 90 天的报价 | 提供方签名、配额、熔断、来源审计 |
| T12 | 单一、示例或过期报价制造虚假精确并触发出售 | 误导决策 | 样本少于 3 或低置信度时不返回单点中位数；出售目标只接受近期同币种非示例报价，不回退到无日期静态估值；保留区间与来源披露 | 用真实已授权样本校准阈值；系统通知上线前做敏感文案审查 |
| T13 | 错误响应回显原始交易或内部堆栈 | 隐私/实现泄露 | 稳定错误码 + request ID；500 使用通用文案 | 集中脱敏日志与响应快照测试 |
| T14 | 依赖、CI 或签名供应链被篡改 | 恶意发行包 | 严格锁文件与 SHA-256 校验、Windows/Android CycloneDX SBOM、OSV 许可证/漏洞审计、固定 action SHA、禁止提交密钥 | 仍需隔离签名 runner、生产证书和独立远端 CI 运行证据 |
