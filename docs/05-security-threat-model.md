...
剩余高风险项：Desktop 已接入 AES-256-GCM 受保护账本及 Windows DPAPI/macOS Keychain 边界，但 Android/Apple 平台真实密钥行为、Android/iOS 旧 Room 库迁移和移动端入口切换尚未完成；批次撤销与用户后续编辑的冲突策略待持久化层实现；沙箱网关为单进程内存 state；服务未配置真实 vault/限流/集中审计；真实平台 scope 尚未取得。这些都必须在对应生产开关开启前关闭或书面接受。
