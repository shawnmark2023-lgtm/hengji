...
输入不包含逐笔账单行、商户名、备注、账户标识、本地实体 ID、导入文件、OCR 原文或短信原文。候选到本地去重键的映射保留在 provider 合同之外。分析偏好、教程完成时间和历史结果自 schema 5 起进入受保护账本；当前 Room/JSON schema 6 另加入用户月预算，并继续由 AES-256-GCM 和平台密钥保护。
